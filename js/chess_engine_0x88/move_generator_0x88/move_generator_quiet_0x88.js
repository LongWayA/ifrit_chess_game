...
import {
    BOARD_SIZE_CB, OUT_OF_BOUNDS_MASK_CB, SIDE_TO_MOVE_CB,
...
import {
    add_packing_move_ml, return_type_simple_move_ml,
     MOVE_PAWN_PROMO_QUEEN_ML, MOVE_PAWN_PROMO_ROOK_ML, MOVE_PAWN_PROMO_BISHOP_ML,
    MOVE_PAWN_PROMO_KNIGHT_ML, MOVE_PAWN_ML,
    MOVE_DOUBLE_PAWN_ML, MOVE_KING_CASTLE_ML, MOVE_KING_QUEEN_CASTLE_ML
    ...
    for (let j = 0; j < 8; j++) {
        ...
        if ((to & OUT_OF_BOUNDS_MASK_CB) == 0) {// если мы не вышли за пределы доски
            while (true) {
                ...
                if ((to & OUT_OF_BOUNDS_MASK_CB) != 0) break;// конец доски
            }
        }
    }
}
