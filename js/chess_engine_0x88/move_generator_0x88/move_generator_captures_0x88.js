...
//+
// Предлагается Оптимизация: Инлайнинг для дальнобойных фигур

import {
    BOARD_SIZE_CB, OUT_OF_BOUNDS_MASK_CB, SIDE_TO_MOVE_CB, LET_COOR_CB,
    BLACK_CB, WHITE_CB, W_PAWN_CB, W_KNIGHT_CB, W_BISHOP_CB, W_ROOK_CB, W_QUEEN_CB, W_KING_CB, B_PAWN_CB,
    B_KNIGHT_CB, B_BISHOP_CB, B_ROOK_CB, B_QUEEN_CB, B_KING_CB, IND_EN_PASSANT_YES_CB,
    IND_EN_PASSANT_TARGET_SQUARE_CB
} from "./chess_board_0x88.js";

import {
    clear_list_ml, add_packing_move_ml, return_type_captures_pawn_promo_ml,
    return_type_simple_move_ml, 
    IND_PIECE_COLOR_ML, IND_NUMBER_CAPTURES_MOVE_ML, IND_NUMBER_MOVE_ML,
...
const generated_pseudo_legal_captures_mgc = function (chess_board_0x88, packing_moves) {
    ...
    packing_moves[IND_PIECE_COLOR_ML] = side_to_move;

    // бежим по доске
    for (let from = 0; from < BOARD_SIZE_CB; from++) {
...
const generated_captures_moves_king_mgc = function (piece_name, piece_color, from, chess_board_0x88, packing_moves) {
    ...
    for (let j = 0; j < 8; j++) {
        ...
        if ((to & OUT_OF_BOUNDS_MASK_CB) == 0) {// если мы не вышли за пределы доски
    ...
    for (let j = 0; j < 8; j++) {
        ...
        if ((to & OUT_OF_BOUNDS_MASK_CB) == 0) {// если мы не вышли за пределы доски
            while (true) {
                ...
                if ((to & OUT_OF_BOUNDS_MASK_CB) != 0) break;// конец доски прерываем поиск на этом луче
            }
        }
    }
}
    ...
    for (let j = 0; j < 4; j++) {
        ...
        if ((to & OUT_OF_BOUNDS_MASK_CB) == 0) {// если мы не вышли за пределы доски
            while (true) {
                ...
                if ((to & OUT_OF_BOUNDS_MASK_CB) != 0) break;
            }
        }
    }
}
...
const generated_moves_pawn_captures_mgc = function (piece_name, from, to_left, to_right, piece_color, chess_board_0x88, packing_moves) {
    ...
    if ((to_left & OUT_OF_BOUNDS_MASK_CB) == 0) {// взятия налево если мы на доске
        ...
        // НЕ ИСПОЛЬЗУЕМ return_type_simple_move_ml так как она в этом случае возвращает MOVE_PAWN_ML
            ...
            // тут тоже особенность piece_captures = 0, так что в реализаторе ходов надо учесть, что при EP_CAPTURES_ML
            // у нас передается пустое поле а не взятая фигура 
            add_packing_move_ml(packing_moves, type_move, from, to_left, piece_captures);
        }
    }
    if ((to_right & OUT_OF_BOUNDS_MASK_CB) == 0) {// 136 0x88    
...
const generated_captures_moves_pawn_promo_mgc = function (piece_name, from, to_left, to_right, piece_color, chess_board_0x88, packing_moves) {
    ...
    if ((to_left & OUT_OF_BOUNDS_MASK_CB) == 0) {// 136 0x88                
        piece_captures = chess_board_0x88[to_left];
        //piece_color_captures = (chess_board_0x88[to_left] > W_KING) ? BLACK : WHITE;
        piece_color_captures = 1 - (chess_board_0x88[to_left] >> 3);
